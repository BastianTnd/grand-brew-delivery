...
@export var arrow_min_distance = 30.0
...
func _physics_process(delta):
	...
	if move_input < 0: current_power = engine_power * 0.3 
	...
	if speed > max_speed: velocity = velocity.limit_length(max_speed)

	var collided = move_and_slide()
	
	if collided and speed > 200: 
...
func apply_crash_logic():
	if crash_cooldown <= 0:
		...
		if crash_counter >= 3:
			crash_counter = 0
			var penalty = max_beer * 0.25
			beer_level = clamp(beer_level - penalty, 0.0, max_beer)
...
func _on_beer_level_changed(new_value):
	if beer_visual:
		var frame_idx = clampi(int(round(new_value / 25.0)), 0, 4)
...
func _get_target_positions() -> Array:
	...
	if ScoreManager.items_collected < 3:
		...
		for item in items: targets.append(item.global_position)
